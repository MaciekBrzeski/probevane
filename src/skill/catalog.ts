...
export const COMMANDS: Command[] = [
  ...
  { name: 'refactor', summary: 'Characterization-first refactor: change source only, every test stays green (behavior_lock). --quality adds a source-quality gate (edited files mustn\'t regress).', usage: 'probevane refactor <dir> --task "<what to refactor>" [--model …] [--budget N] [--quality]', example: 'probevane refactor ./app --task "extract helpers into utils.ts" --quality' },
  { name: 'feature', summary: 'TDD red-first: write a failing test, implement, go green; existing tests protected. --quality gates edited-source quality.', usage: 'probevane feature <dir> --task "<feature>" [--model …] [--quality]', example: 'probevane feature ./app --task "add a discount field to cartTotal"' },
  { name: 'repair', summary: 'After source changes, update the affected (stale) tests so the whole suite is green. --quality gates edited-source quality.', usage: 'probevane repair <dir> [--since <ref>] [--model …] [--quality]', example: 'probevane repair ./app --since HEAD~1' },
  { name: 'fix', summary: 'Apply described issues/findings to the code, keeping the suite green + audit-clean. --quality gates edited-source quality.', usage: 'probevane fix <dir> --task "<issues>" [--model …] [--quality]', example: 'probevane fix ./app --task "handle the null case in parse()"' },
