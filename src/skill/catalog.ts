...
export const COMMANDS: Command[] = [
  ...
  { name: 'generate', summary: 'Probe-grounded gated loop writes unit/e2e tests (mock maker, hermetic, audited).', usage: 'probevane generate <dir> [--kind unit|e2e] [--model auto|haiku|sonnet|opus|local:<id>] [--mock] [--mutation] [--flake-guard] [--target-gaps] [--a11y] [--visual] [--quality] [--assert-min N] [--flake-tolerance K] [--passk N] [--budget N] [--only <substr>] [--spec]', example: 'probevane generate ./my-app --kind unit --mock' },
  ...
  { name: 'repair', summary: 'After source changes, update the affected (stale) tests so the whole suite is green. --quality gates edited-source quality.', usage: 'probevane repair <dir> [--since <ref>] [--only <path>] [--model …] [--force-stop-after N] [--quality] [--mfe]', example: 'probevane repair ./app --since HEAD~1' },
  { name: 'fix', summary: 'Apply described issues/findings to the code, keeping the suite green + audit-clean. --quality gates edited-source quality.', usage: 'probevane fix <dir> --task "<issues>" [--only <path>] [--model …] [--force-stop-after N] [--quality] [--mfe]', example: 'probevane fix ./app --task "handle the null case in parse()"' },
