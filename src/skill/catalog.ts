...
export const COMMANDS: Command[] = [
  ...
  { name: 'refactor', summary: 'Characterization-first refactor: change source only, every test stays green (behavior_lock). --quality adds a source-quality gate (edited files mustn\'t regress).', usage: 'probevane refactor <dir> --task "<what to refactor>" [--model …] [--budget N] [--force-stop-after N] [--quality]', example: 'probevane refactor ./app --task "extract helpers into utils.ts" --quality' },
  { name: 'feature', summary: 'TDD red-first: write a failing test, implement, go green; existing tests protected. --quality gates edited-source quality.', usage: 'probevane feature <dir> --task "<feature>" [--model …] [--force-stop-after N] [--quality]', example: 'probevane feature ./app --task "add a discount field to cartTotal"' },
  { name: 'repair', summary: 'After source changes, update the affected (stale) tests so the whole suite is green. --quality gates edited-source quality.', usage: 'probevane repair <dir> [--since <ref>] [--model …] [--force-stop-after N] [--quality]', example: 'probevane repair ./app --since HEAD~1' },
  { name: 'fix', summary: 'Apply described issues/findings to the code, keeping the suite green + audit-clean. --quality gates edited-source quality.', usage: 'probevane fix <dir> --task "<issues>" [--model …] [--force-stop-after N] [--quality]', example: 'probevane fix ./app --task "handle the null case in parse()"' },
  ...
  { name: 'factory', summary: 'Run the gated generate loop over many repos concurrently — each with isolated state, errored repos auto-reverted — into one cost/coverage/quality rollup (report.json). Unrecognized flags forward to generate per-repo.', usage: 'probevane factory <repos.txt | dir...> [--concurrency N] [--kind unit|e2e] [--report <path>] [--state-root <dir>] [--no-checkpoint] [...generate flags]', example: 'probevane factory repos.txt --concurrency 4 --model auto' },
  { name: 'quality', summary: 'Project source-quality gate — file size, function length/complexity/nesting/params, long lines, debt markers (TODO/FIXME), import fan-out, and duplication → a 0–100 health grade. --strict exits 1 on error-severity violations (CI). Complements audit (test specs), assert-score (assertions), bench (mutation).', usage: 'probevane quality <dir> [--strict] [--json] [--max-file N] [--max-fn N] [--max-complexity N] [--max-nesting N] [--max-params N] [--max-width N] [--max-imports N] [--no-debt]', example: 'probevane quality ./app --strict' },
  { name: 'daemon', summary: 'Long-running OPERATE/OBSERVE service — scans every ledger under the state root and serves /health, /aggregate (cost+acceptance over time), /alerts (cost spike / acceptance drop / error burst), /audit (library-mutation trail) over HTTP; periodically re-evaluates alerts to a structured log. Graceful shutdown; read-only over state.', usage: 'probevane daemon [--port N] [--root <stateDir>] [--interval SEC]', example: 'probevane daemon --port 7766' },
