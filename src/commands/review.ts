...
// `probevane review` backend — read-only quality grade for an existing test
// suite: green, coverage, audit cleanliness, flake → a 0–100 grade + report.
// No generation — works on any repo today. Logic moved verbatim from the old
// src/cli/review.ts shell; the vane interpreter owns argv.

/** Grade the suite in ctx.dir and emit the markdown report (+ GHA summary). */
export async function run(ctx: CommandCtx): Promise<void> {
  const dir = ctx.dir;
  const flakeRuns = ctx.flags.flake as number;
