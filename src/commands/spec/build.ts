...
export async function buildSpec(opts: SpecOptions): Promise<string> {
  ...
  const cfg = await loadConfig(dir).catch((): ProbevaneConfig => ({}));
  ...
  lines.push(...oraclesLines(cfg.oracles));
...
/** The `## Acceptance oracles` section (ADR-022): the declared per-module correctness
 *  contracts from probevane.config. Empty when none are declared. */
function oraclesLines(oracles?: Record<string, OracleSpec>): string[] {
  const entries = Object.entries(oracles ?? {});
  if (!entries.length) return [];
  const lines = ['## Acceptance oracles', '', '| Module | Kind | Contract | Blocking |', '|---|---|---|---|'];
  for (const [mod, o] of entries)
    lines.push(`| ${mod} | ${o.kind} | ${o.desc} | ${o.blocking ? 'yes' : ''} |`);
  lines.push('');
  return lines;
}
