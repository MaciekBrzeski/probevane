...
async function main() {
  ...
  const outcome = await runPath({ dir, adapter, profileName: "refactor", task: fullTask, model, maxSteps, budget, quality, mfe, forceStopAfter, only, log: (l) => console.error(l) });
  console.log(
