...
async function main() {
  ...
  const outcome = await runPath({ dir, adapter, profileName: "refactor", task: fullTask, model, maxSteps, budget, quality, mfe, forceStopAfter, only, worktree: args.includes("--worktree"), worktreeMerge: args.includes("--worktree-merge"), worktreeReview: args.includes("--worktree-review"), log: (l) => console.error(l) });
  console.log(
