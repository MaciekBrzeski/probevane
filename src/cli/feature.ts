...
async function main() {
  ...
  const outcome = await runPath({ dir, adapter, profileName: "feature", task: fullTask, model, maxSteps, budget, quality, mfe, forceStopAfter, only, worktree: args.includes("--worktree"), worktreeMerge: args.includes("--worktree-merge"), log: (l) => console.error(l) });
  console.log(
