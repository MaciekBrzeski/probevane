...
/** ⟲ HISTORY: list the current project's past runs in the signals region. */
export async function showHistory(): Promise<void> {
  ...
  let runs: RunRecord[] = [];
  try { runs = ((await j('/runs?limit=300')) as { runs: RunRecord[] }).runs; } catch { /* offline */ }
  const mine = runsForDir(runs, dir);
  if (!mine.length) { setSignals(empty(`no past runs recorded for ${baseName(dir)}`)); return; }
  const box = <div></div> as HTMLElement;
  box.appendChild(<div class="chat-signal-sub">past runs · {baseName(dir)} ({mine.length})</div>);
  for (const r of mine.slice(0, 30)) box.appendChild(runHistoryCard(dir, r));
  setSignals(box);
}
