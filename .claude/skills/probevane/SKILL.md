...
| `quality` | Project source-quality gate — file size, function length/cyclomatic+cognitive complexity/nesting/params, long lines, debt markers (TODO/FIXME, comment-scoped), import fan-out, (maximal-block) duplication, and doc coverage (doc-comment: a function should say what it does and why; type-doc: an exported interface/type should carry a shape comment) → a 0–100 health grade. Size rules count CODE lines only — comment-only lines never push a file/function over the bar, so the gate rewards documenting instead of punishing it. --strict exits 1 on error-severity violations (CI); --sarif emits GitHub code-scanning output; --write-baseline ratchets (gate only NEW violations on a messy repo, --no-baseline ignores it); --since <ref> scans only changed files; --no-docs disables the doc rules. Complements audit (test specs), assert-score (assertions), bench (mutation). |
...
# e.g.
probevane visual ./app --url http://localhost:5173 --goal "water renders with animated waves, no z-fighting"
```
...
# e.g.
probevane plan-feature --from "todo app, no persistence" --to "todos survive reload via localStorage"
```
...
# e.g.
probevane design --url http://localhost:7766/ --target src/ui/control.html --tabs projects,runs,docs --spec e2e-dash/control-shots.spec.ts
```
...
### quality
Project source-quality gate — file size, function length/cyclomatic+cognitive complexity/nesting/params, long lines, debt markers (TODO/FIXME, comment-scoped), import fan-out, (maximal-block) duplication, and doc coverage (doc-comment: a function should say what it does and why; type-doc: an exported interface/type should carry a shape comment) → a 0–100 health grade. Size rules count CODE lines only — comment-only lines never push a file/function over the bar, so the gate rewards documenting instead of punishing it. --strict exits 1 on error-severity violations (CI); --sarif emits GitHub code-scanning output; --write-baseline ratchets (gate only NEW violations on a messy repo, --no-baseline ignores it); --since <ref> scans only changed files; --no-docs disables the doc rules. Complements audit (test specs), assert-score (assertions), bench (mutation).

```
probevane quality <dir> [--strict] [--json] [--sarif] [--write-baseline] [--no-baseline] [--since <ref>] [--max-file N] [--max-fn N] [--max-complexity N] [--max-cognitive N] [--max-nesting N] [--max-params N] [--max-width N] [--max-imports N] [--no-debt] [--no-docs]
# e.g.
