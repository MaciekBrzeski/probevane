...
**Drawn by the shared engine.** The panes render through the [Drawing Engine (facet)](Drawing-Engine.md) via a `paintWidget` bridge — the *same* component library the browser uses. Projects is a `card` grid (run-count `badge` + accept-rate `progress`); Launch is a `menu`; the Console hero carries `stat` blocks + a run-phase `stepper` + a recent-runs `timeline` under the pipeline lamps; Cost adds a daily-cost `barChart` + an acceptance `donut` + `legend`; empty panes show an `emptyState`. Gauges, sparklines, and the node-graph are the engine's terminal backend of the browser's SVG widgets — one authoring, two media.
