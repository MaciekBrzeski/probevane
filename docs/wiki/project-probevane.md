...
- test coverage: 63.34% statements, 82.95% branches
...
Defines accessibility audit rules returned as an array of AuditRule definitions.
...
Provides the base adapter interface and shared types for framework-specific test adapters.

### src/util/exec.ts — util

Executes shell commands with timeout and captures capped stdout/stderr output.
...
Defines JavaScript-specific static audit rules for code quality checks.
...
Extracts AST facts from source files for static analysis probes.
...
Loads named prompt templates from the prompt library asynchronously.
...
Reads and parses coverage summary data from a coverage output directory.
...
Exports the Angular adapter integrating detection, discovery, installation, and test execution.
...
Defines Go-specific static audit rules for code quality checks.
...
Exports the Go test adapter integrating detection, discovery, installation, and test execution.
...
Runs Vitest and Playwright tests, merges results, and collects coverage.
...
Finds spec/test files within a directory and optional subpath.
...
Reads package.json dependencies and devDependencies into a version map.
...
Exports the Node.js Vitest adapter for generic Node projects.
...
Provides a no-op adapter used as a fallback when no framework matches.
...
Defines Python-specific static audit rules for code quality checks.
...
Exports the Python pytest adapter integrating detection, discovery, installation, and test execution.
...
Detects React project presence and returns a confidence score.
...
Discovers React unit and e2e test targets within a project directory.
...
Installs and configures Vitest and Playwright dependencies for React projects.
...
Runs React unit and e2e tests and collects coverage.
...
Probes individual React test targets to assess coverage and quality.
...
Exports the React adapter integrating detection, discovery, installation, probing, and execution.
...
Installs and configures Vitest and Playwright dependencies for Vue projects.
...
Probes a project directory to discover Vue test targets and determine the applicable test stack.
...
Executes Vue unit and Playwright e2e tests and collects coverage results for a given directory.
...
Provides Vue-specific audit rules for static source analysis.
...
Exports the Vue adapter object combining probe, run, and coverage capabilities.
...
Runs Svelte Vitest tests and gathers coverage metrics for a project directory.
...
Exports the Svelte adapter object wiring together probe and run functions.
...
Provides Rust-specific audit rules for static source analysis.
...
Exports the Rust Cargo adapter object integrating probe and run capabilities.
...
Maintains the list of stack adapters and selects the matching adapter for a given project directory.
...
Scores assertion quality in test source files and aggregates per-file scores.
...
Runs audit rules against source files to detect violations and format the results.
...
Provides a rate-limiting utility to throttle API calls.
...
Wraps the Anthropic SDK into a Brain interface with message conversion and response parsing.
...
Defines the core Brain abstraction for model-driven decision making.

### src/util/state.ts — util

Resolves local state directory and file paths for persisted tool state.
...
Estimates token counts for request inputs and outputs to project usage costs.
...
Bridges a generic response into the Brain interface for non-LLM or fallback execution.
...
Extracts and validates JSON values from model output text.
...
Implements a Brain backed by Claude Code with decision parsing and JSON extraction.
...
Implements a Brain for OpenAI-compatible APIs with message mapping and response conversion.
...
Provides recording and replaying Brain wrappers for deterministic test cassettes.
...
Selects the appropriate Brain implementation for a given model name.
...
Provides accessibility-related CLI utilities and checks.

- depends on: src/audit/core.ts, src/a11y/rules.ts

### src/integrations/ado.ts — util

Implements an Azure DevOps client with WIQL, patch, and directive parsing helpers.
...
Provides CLI commands for Azure DevOps integration.

- depends on: src/integrations/ado.ts

### src/cli/assert-score.ts — util

Provides a CLI command to assert and report test suite scores.

- depends on: src/adapters/registry.ts, src/audit/assertion-score.ts

### src/cli/audit.ts — util

Provides a CLI command to audit the repository for probevane configuration and health.

- depends on: src/adapters/registry.ts, src/audit/core.ts

### src/loop/passk.ts — util

Implements pass@k generation by scoring suites and selecting the best candidate across multiple attempts.
...
Performs mutation testing by generating mutants, detecting survivors, and computing a mutation score.
...
Provides a CLI command to benchmark probevane operations and measure performance.

- depends on: src/adapters/registry.ts, src/loop/passk.ts, src/loop/mutation.ts

### src/loop/triage.ts — util

Routes test targets into easy and hard buckets based on source fact density, line count, and imports.
...
Simulates and reports cost savings across caching strategies for a given codebase.
...
Loads, validates, and resolves probevane configuration from a project directory.
...
Identifies changed source files and spec candidates relative to a git reference.
...
Extracts a git diff, sends it to a brain for review, and formats findings as markdown or task output.
...
Grounds findings to actual files and verifies each finding against the diff using a brain.
...
Provides a CLI command to run probevane checks in a CI pipeline context.
...
Provides a CLI command to collect and report test coverage metrics.

- depends on: src/adapters/registry.ts

### src/util/jsonl.ts — util

Appends and reads JSON Lines files for persistent record storage.
...
Tracks consecutive run errors and quarantines flaky targets with exponential backoff.
...
Manages the job queue by deduplicating items, selecting the next ready item, and summarizing queue status.
...
Reduces persisted jobs, selects jobs to evict, and converts launch plans into queue items.
...
Validates incoming launch request bodies against allowed operations and returns a launch result.
...
Tracks spending since a timestamp and enforces budget caps over a rolling time window.
...
Wraps git operations including worktree management, branching, committing, merging, diffing, and file restoration.
...
Generates branch names, PR titles, and PR bodies from ship info and opens pull requests via git.
...
Reads the latest diary record and ships a run by creating a branch, committing, and opening a PR.
...
Formats SSE frames, tails log content, and derives the latest event and status per run.
...
Manages the daemon lifecycle including job launching, cancellation, queueing, supervision, and event streaming.
...
Looks up per-model token rates and computes the cost of a usage record.
...
Persists and summarizes run cost records from a JSON ledger file.
...
Buckets run records into per-day time-series aggregates.
...
Computes alerts from daily buckets and decides whether to halt the loop.
...
Appends and reads structured audit log entries to a file.
...
Converts run records into OpenTelemetry traces, metrics, and Prometheus text payloads.
...
Strips source to code and detects function-level metrics.
...
Builds file- and function-level quality violations against a config.
...
Analyzes project source for quality issues, duplication, and formatting.
...
Selects input paths and scans a project directory to produce a quality report.
...
Initializes and handles HTTP routes for the daemon server.
...
Deduplicates alerts and builds Slack notification payloads.
...
Runs the daemon server process exposing loop control endpoints.
...
Redacts, records, and reads execution traces for distillation.
...
Builds, splits, and summarizes chat examples from traces.
...
Generates base-model completions and scores their value for distillation.
...
CLI entrypoint orchestrating trace collection and dataset building.
...
Builds a module dependency graph and resolves local imports.
...
Assesses target complexity and routes models accordingly.
...
Defines the shared RunCtx context type for loop execution.
...
Evaluates runes to produce allow/block decisions before tool calls and stops.
...
Executes tool calls and provides tool specs for the loop.
...
Formats and parses loop event messages.
...
Extracts test blocks and derives conventional spec paths from outputs.
...
Runs core loop phases including system assembly, event emission, and stepping.
...
Detects circular difficulty signals and proposes remediation.
...
Provides escalation checks (nudge, stuck, budget, difficulty, etc.) that detect when a loop run needs to change strategy.
...
Orchestrates the core loop execution, running segments and caching stable indices for repeatable outcomes.
...
Builds a concise digest of documentation files in a directory for context injection.
...
Defines a rune that enforces planning before implementation in a loop run.
...
Defines a rune that records a session diary of loop activity for traceability.
...
Persists and retrieves example artifacts and the library index from disk.
...
Reads recent caveat notes from the library to surface known pitfalls.
...
Defines a rune that harvests caveats from a loop run into the library.
...
Provides runes for injecting docs context, guarding doc scope/structure, extracting references, and enforcing docs acceptance criteria.
...
Runs a documentation-focused loop using docs-specific runes and options.
...
CLI entry point that wires docs loop commands to the runDocs engine.

- depends on: src/config.ts, src/loop/run-docs.ts

### src/loop/runes/index.ts — util

Barrel module exporting all available runes for loop composition.
...
Maps named profiles to ordered rune segments for loop configuration.
...
Locates a target node in a module graph and produces a focused repo-map directive.
...
Builds a dependency digest scoped to a specific path for context injection.
...
Executes a loop run inside an isolated git worktree and returns the outcome.
...
Runs a path-scoped loop using scout focus and dependency digests.
...
CLI wrapper that parses flags and invokes path-scoped loop runs.
...
CLI entry point that wires documentation generation commands.

- depends on: src/cli/path-cli.ts

### src/cli/enqueue.ts — util

CLI entry point that enqueues loop runs for batch processing.
...
Appends and reads structured improvement log rows for tracking library changes.
...
CLI entry point that runs evaluation suites against loop outcomes.

- depends on: src/adapters/registry.ts, src/library/improvement-log.ts

### src/util/concurrent.ts — util

Runs async worker functions over items with a configurable concurrency limit.
...
Aggregates factory repo results into a report and parses/slugifies repo lists.
...
Detects and parses module federation configuration from source text.
...
Audits microfrontend repositories for standards compliance and version alignment across shared dependencies.
...
Scans directories for module federation configurations and collects shared dependency information across microfrontend repos.
...
Orchestrates the factory execution pipeline and produces a structured report of the run.
...
Builds a test execution matrix from a list of repository paths.
...
Provides the CLI command interface for invoking the factory pipeline.
...
Provides the CLI command interface for feature-related operations.

- depends on: src/cli/path-cli.ts

### src/cli/fix.ts — util

Provides the CLI command interface for applying fixes.

- depends on: src/cli/path-cli.ts

### src/loop/runes/validation_gate.ts — util

Validates test run output by detecting TypeScript errors, extracting first failures, and gating the loop on validation results.
...
Produces a compact, capped digest of source or facts for use as context.
...
Provides guidance for property-based testing and heuristically detects whether source is amenable to property tests.
...
Drafts and iteratively repairs tests locally for a target module using a brain and stack adapter.
...
Retrieves few-shot examples matching a retrieval query from the example library.
...
Tokenizes strings, computes set-based similarity, and selects the most similar trace for a given task and stack.
...
Injects mock setup runes into a test plan based on a mock plan.
...
Synthesizes mock bundles for modules by loading OpenAPI specs and sampling schema-compatible fixture data.
...
Materializes fixtures, captures API contracts from module graphs, and persists/reads contract files on disk.
...
Writes mock handlers and fixtures to disk as a runnable mock server.
...
Orchestrates mock plan building and module graph construction, re-exporting core mock types and graph utilities.
...
Runs the full test generation loop and returns the overall outcome.
...
Provides the CLI command interface for invoking test generation.
...
Renders module graphs as Mermaid diagrams, ASCII art, or concise text summaries.
...
Provides the CLI command interface for rendering module dependency graphs.

- depends on: src/mock/index.ts, src/mock/render.ts

### src/cli/history.ts — util

Provides the CLI command interface for viewing run history.

- depends on: src/cost/ledger.ts

### src/loop/impact.ts — util

Computes which specs are impacted by changed modules using graph reachability analysis.
...
Provides the CLI command interface for computing and displaying impact analysis.

- depends on: src/adapters/registry.ts, src/git.ts, src/mock/graph.ts, src/loop/impact.ts

### src/distill/improve.ts — util

Provides utilities for the distillation improvement cycle, including cycle scheduling checks, evaluation comparison, and model pointer persistence.
...
CLI command that orchestrates and runs the distillation improvement cycle.

- depends on: src/distill/improve.ts

### src/visual/capture.ts — util

Captures screenshots of a given URL and saves them to an output path.
...
Sends captured screenshots with system/user prompts to a vision model and returns its textual response.
...
Runs an iterative visual improvement loop that captures screenshots, asks a vision model for feedback, and applies suggested changes.
...
CLI command that initiates and manages the visual improvement workflow.

- depends on: src/visual/improve.ts

### src/cli/init.ts — util

CLI command that initializes project configuration and scaffolding.

- depends on: src/adapters/registry.ts

### src/cli/learn.ts — util

CLI command that drives the learning or training data collection workflow.

- depends on: src/adapters/registry.ts, src/audit/core.ts, src/library/store.ts

### src/cli/mfe-audit.ts — util

CLI command that audits micro-frontend modules for quality and compliance issues.

- depends on: src/mfe/scan.ts, src/mfe/standards.ts, src/factory/report.ts

### src/mfe/contract.ts — util

Generates contract test files for exposed and consumed micro-frontend module interfaces.
...
Scans a directory to plan and discover micro-frontend contracts.
...
CLI command that generates or validates micro-frontend contracts.

- depends on: src/mfe/contract-scan.ts

### src/mfe/report.ts — util

Aggregates micro-frontend driver results and version-alignment violations into a final report.
...
Orchestrates micro-frontend analysis runs and produces a driver report.
...
CLI command that runs micro-frontend analysis and reporting.

- depends on: src/mfe/driver.ts, src/factory/report.ts, src/mfe/standards.ts

### src/cli/migrate.ts — util

CLI command that performs project migrations or transformations.

- depends on: src/cli/path-cli.ts

### src/cli/mock.ts — util

CLI command that generates or manages mock data and fixtures.

- depends on: src/mock/index.ts

### src/cli/otel.ts — fetcher 🌐

CLI command that fetches and surfaces OpenTelemetry observability data.

- depends on: src/cost/ledger.ts, src/util/state.ts, src/observe/otel.ts

### src/cli/peek.ts — util

CLI command that inspects and displays current project or pipeline state.

- depends on: src/loop/events.ts, src/loop/observe.ts

### src/loop/rune-descriptions.ts — util

Provides descriptive metadata and human-readable descriptions for pipeline runes and hooks.
...
Builds and renders descriptions of the pipeline structure, including profiles, runes, phases, toggles, and Mermaid diagrams.
...
CLI command that describes or inspects the configured pipeline.

- depends on: src/loop/describe.ts

### src/plan/build.ts — util

Constructs a project plan from inputs, identifying untested targets and formatting the plan for output.
...
CLI command that generates and displays a project plan.

- depends on: src/adapters/registry.ts, src/quality/scan.ts, src/mfe/scan.ts, src/plan/build.ts

### src/quality/baseline.ts — util

Manages quality violation baselines by writing, keying, and applying baseline reports to filter known issues.
...
Converts a QualityReport into the SARIF JSON format for standardized static analysis output.
...
Provides the CLI command entry point for running quality analysis on the project.
...
Provides the CLI command entry point for triggering automated refactoring operations.

- depends on: src/cli/path-cli.ts

### src/cli/repair.ts — util

Provides the CLI command entry point for running automated code repair workflows.

- depends on: src/cli/path-cli.ts, src/git.ts

### src/cli/revert.ts — util

Provides the CLI command entry point for reverting changes to a previous checkpoint or state.

- depends on: src/util/git.ts

### src/cli/review.ts — util

Provides the CLI command entry point for performing code review analysis.

- depends on: src/adapters/registry.ts, src/audit/core.ts

### src/cli/run.ts — util

Provides the CLI command entry point for executing the main agent or pipeline run.

- depends on: src/adapters/registry.ts

### src/cli/scan.ts — util

Provides the CLI command entry point for scanning the codebase for issues or metadata.

- depends on: src/util/state.ts, src/util/jsonl.ts, src/observe/queue.ts, src/factory/report.ts

### src/cli/serve.ts — util

Provides the CLI command entry point for starting a local server (e.g., dashboard or API).

- depends on: src/loop/observe.ts

### src/cost/project.ts — util

Projects future cost ledgers from run records, including free-run detection and formatting utilities.
...
Provides the CLI command entry point for simulating and estimating project costs.
...
Maintains the catalog of available skill commands, including documented and undocumented entries.
...
Builds skill definition files and wiki command documentation from the skill catalog.
...
Provides the CLI command entry point for managing and invoking skills.

- depends on: src/skill/build.ts, src/skill/catalog.ts

### src/spec/build.ts — util

Assembles a specification document from provided spec options into a single string output.
...
Provides the CLI command entry point for generating or building project specifications.

- depends on: src/adapters/registry.ts, src/brain/select.ts, src/spec/build.ts

### src/cli/status.ts — util

Provides the CLI command entry point for displaying current project or run status.

- depends on: src/adapters/registry.ts

### src/cli/watch.ts — util

Provides the CLI command entry point for watching files and triggering actions on changes.

- depends on: src/adapters/registry.ts, src/config.ts, src/git.ts

### src/e2e/checkpoint.ts — util

Captures a named e2e test checkpoint snapshot on a Playwright page with configurable options.
...
Executes a delegated loop run with the given options and returns a structured outcome.
...
Defines an accessibility gate rune that evaluates a11y compliance as a loop checkpoint.
...
Creates an acceptance gate rune that validates acceptance criteria within the loop.
...
Creates an assertion gate rune enforcing a minimum assertion count threshold.
...
Defines an audit gate rune that enforces audit-level quality checks in the loop.
...
Creates a behavior lock rune that prevents regressions in expected behaviors during the loop.
...
Provides a rune that injects appropriate execution context metadata for unit or e2e test kinds.
...
Provides a rune that distills execution traces into a compact, structured object representation.
...
Provides a rune and verdict helper that detect flaky test signatures across repeated runs within a given tolerance.
...
Provides a rune that enforces hermetic (isolated) execution conditions for the loop.
...
Provides helpers and a rune that classify, score, slug, and promote validated specs into the shared library.
...
Provides a rune that gates progression based on micro-frontend design system compatibility.
...
Provides a rune that optionally enforces a minimum mutation testing score and mutant count threshold.
...
Provides a rune that blocks progression when regressions are detected.
...
Provides a rune that validates and guards file path conventions within the loop.
...
Provides a configurable rune that enforces overall quality thresholds with optional overrides.
...
Provides a rune that enforces the red-first (failing test before implementation) discipline.
...
Provides a rune that gates progression on visual regression or snapshot checks.
...
Defines shared type definitions used across the loop and its runes.
