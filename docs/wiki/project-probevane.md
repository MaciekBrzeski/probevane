...
- test coverage: 92.23% statements, 91.23% branches
...
Provides accessibility audit rules as a list of AuditRule definitions.
...
Defines the shared adapter interface and base types used across all framework adapters.

### src/util/exec.ts — util

Executes shell commands with timeout and output capping utilities.
...
Provides JavaScript-specific audit rules for static analysis checks.
...
Extracts lightweight AST facts from source files for adapter probing.
...
Loads named prompt templates from the prompt library.
...
Reads and parses coverage summary reports from a coverage output directory.
...
Exports the Angular adapter object implementing the adapter contract for Angular projects.
...
Provides Go-specific audit rules for static analysis checks.
...
Exports the Go adapter object implementing the adapter contract for Go projects using `go test`.
...
Runs Vitest and Playwright test suites, merges results, and collects coverage.
...
Finds test spec files within a directory for a given subject scope.
...
Reads a project's package.json and returns its dependency map.
...
Exports the Node adapter object implementing the adapter contract for Node.js projects using Vitest.
...
Exports a no-op adapter used as a fallback when no framework matches.
...
Provides Python-specific audit rules for static analysis checks.
...
Exports the Python adapter object implementing the adapter contract for Python projects using pytest.
...
Detects the presence and confidence level of a React project in a directory.
...
Discovers test targets in a React project for a given test kind.
...
Installs required dependencies and configuration for React testing with Vitest and Playwright.
...
Runs React unit and end-to-end tests and collects coverage.
...
Probes individual React test targets to determine unit and e2e probe results.
...
Exports the React adapter object implementing the adapter contract for React projects using Vitest and Playwright.
...
Installs required dependencies and configuration for Vue testing with Vitest and Playwright.
...
Probes a directory to discover Vue test targets for Vitest and Playwright.
...
Runs Vue test suites and collects coverage results via Vitest and Playwright.
...
Provides Vue-specific audit rules for static analysis violations.
...
Exports the combined Vue adapter object integrating probe, run, and coverage capabilities.
...
Runs Svelte test suites and gathers coverage results using Vitest.
...
Exports the Svelte adapter object integrating Vitest-based test execution.
...
Provides Rust-specific audit rules for static analysis violations.
...
Exports the Rust adapter object integrating Cargo-based test and build capabilities.
...
Registers available stack adapters and selects the appropriate one for a given directory.
...
Scores assertion quality in source files and aggregates per-file scores into a summary.
...
Audits source files against rules to produce violation reports and formatted output.
...
Implements an Anthropic SDK-based brain with streaming buffer flushing and message conversion.
...
Defines the core Brain abstraction for LLM-driven decision making.

### src/util/state.ts — util

Resolves local state directory paths for persisted runtime data.
...
Estimates token counts for request inputs and outputs including tool calls.
...
Bridges brain responses into a normalized BrainResponse with token estimates.
...
Extracts and validates JSON values from LLM text output.
...
Implements a Claude Code brain that parses decisions and tool calls from model output.
...
Implements an OpenAI-compatible brain with message conversion and response mapping.
...
Provides recording and replay brains for deterministic LLM cassette-based testing.
...
Selects the appropriate brain implementation for a given model identifier.
...
Implements CLI accessibility checks and reporting.

- depends on: src/audit/core.ts, src/a11y/rules.ts

### src/integrations/ado.ts — util

Provides Azure DevOps client utilities for work item updates, auth, and directive parsing.
...
Implements the CLI command for Azure DevOps integration.

- depends on: src/integrations/ado.ts

### src/cli/assert-score.ts — util

Provides CLI logic for asserting and reporting test suite scores.

- depends on: src/adapters/registry.ts, src/audit/assertion-score.ts

### src/cli/audit.ts — util

Implements the CLI audit command for inspecting project health and configuration.

- depends on: src/adapters/registry.ts, src/audit/core.ts

### src/loop/passk.ts — util

Scores test suites and selects the best candidate using pass@k generation strategies.
...
Runs mutation testing to compute mutation scores and identify surviving mutants.
...
Implements the CLI benchmarking command for measuring performance.

- depends on: src/adapters/registry.ts, src/loop/passk.ts, src/loop/mutation.ts

### src/loop/triage.ts — util

Classifies test targets into easy and hard buckets based on source complexity heuristics.
...
Simulates caching strategy costs and formats savings reports for a codebase.
...
Loads, validates, and picks configuration values for the tool.
...
Detects changed source files and maps them to candidate spec files via git.
...
Extracts diffs, runs AI review to produce findings, and formats them as markdown or task lists.
...
Grounds and verifies review findings against diffs to confirm their validity.
...
Implements the CI integration command for running checks in a pipeline.
...
Implements the CLI command for collecting and reporting test coverage.

- depends on: src/adapters/registry.ts

### src/util/jsonl.ts — util

Provides append and read helpers for JSON Lines files.
...
Tracks consecutive run errors and applies quarantine with exponential backoff.
...
Manages queue items lifecycle including creation, status marking, readiness, and summary counts.
...
Reduces persisted jobs, determines eviction candidates, and converts launch plans into queue items.
...
Validates incoming launch requests against allowed operations.
...
Tracks spending over time windows and enforces cost caps.
...
Wraps common git operations including worktrees, branches, diffs, commits, and file restoration.
...
Generates branch names, PR titles and bodies, and opens pull requests.
...
Reads the latest diary record and ships a run result to a pull request.
...
Provides SSE framing, log tailing, and run status derivation for live observation.
...
Controls the daemon server managing jobs, queue, supervision, and event streaming.
...
Looks up per-model token rates and computes usage cost.
...
Records, reads, and summarizes run cost ledger entries.
...
Buckets run records by day and aggregates metrics over time.
...
Appends and reads audit log entries to a persistent file.
...
Constructs quality violations at file and function levels against a config.
...
Analyzes files and the project for quality issues, duplication, and formatting.
...
Deduplicates new alerts and builds Slack notification payloads.
...
Runs the daemon server process hosting HTTP routes.
...
Redacts text and records/reads execution traces for distillation.
...
Builds, splits, and summarizes chat examples from traces for fine-tuning.
...
Generates baseline completions and computes base values for distillation.
...
CLI entrypoint orchestrating the distillation dataset and training workflow.
...
Defines the shared run context type for loop execution.
...
Evaluates rune guardrails to block or allow tool calls and stops.
...
Executes tool calls and provides tool specifications for the loop.
...
Formats and parses loop events to and from text.
...
Extracts test blocks from text and resolves conventional spec paths.
...
Assembles system prompts, manages cache pruning, and runs individual loop steps.
...
Provides heuristic checks (nudge, stuck, budget, difficulty, consult, never-edited) that decide when a loop run should escalate or abort.
...
Orchestrates the core autonomous loop run, driving rune segments to completion and returning a structured outcome.
...
Builds a compact textual digest of a docs directory for injection into loop context.
...
Defines a rune that forces the loop to produce a plan before any implementation step.
...
Defines a rune that appends a running diary of session actions and observations to the loop transcript.
...
Persists and retrieves example artifacts and the library index from the on-disk example library.
...
Reads recent caveat entries from the caveats log file for harvest and review.
...
Defines a rune that collects and surfaces caveats discovered during a loop run for library promotion.
...
Provides runes for injecting docs context, guarding doc scope, validating structure and references, and enforcing docs acceptance criteria.
...
Runs a documentation-focused loop variant that produces or updates docs using rune-driven segments.
...
Implements the CLI command surface for invoking documentation loop runs.

- depends on: src/config.ts, src/loop/run-docs.ts

### src/loop/runes/index.ts — util

Re-exports the full catalog of available runes for composition into loop profiles.
...
Maps named profiles to ordered arrays of rune segments with configurable options.
...
Locates a target module in a module graph and produces a focused repo-map directive for the loop.
...
Builds a dependency digest for a given directory filtered to a specific path for loop context.
...
Executes a loop run inside an isolated git worktree and returns its outcome.
...
Drives a single path-oriented loop run targeting a specific module or dependency path.
...
Parses CLI flags and launches path-targeted loop runs for a given profile and build task.
...
Implements the CLI command surface for documenting a codebase via the loop.

- depends on: src/cli/path-cli.ts

### src/cli/enqueue.ts — util

Implements the CLI command surface for enqueueing loop runs into a queue.
...
Appends and reads structured improvement-log rows used to track library promotion history.
...
Implements the CLI command surface for evaluating loop outcomes and metrics.

- depends on: src/adapters/registry.ts, src/library/improvement-log.ts

### src/util/concurrent.ts — util

Runs an array of async items through a worker with a bounded concurrency pool.
...
Aggregates factory repo results into a report, tracking accepted repos and parsing repo lists.
...
Detects and parses module-federation configuration text from build files.
...
Audits microfrontend repositories for framework and version alignment and formats violations for reporting.
...
Reads module federation configs and collects shared dependency metadata across microfrontend repositories.
...
Executes the factory pipeline and returns a report of generated or processed outcomes.
...
Builds a matrix of repositories for factory orchestration.
...
Provides the CLI command entry point for running the factory workflow.
...
Provides the CLI command entry point for feature-related operations.

- depends on: src/cli/path-cli.ts

### src/cli/fix.ts — util

Provides the CLI command entry point for applying automated fixes.

- depends on: src/cli/path-cli.ts

### src/loop/runes/validation_gate.ts — util

Enforces validation gates within the loop by detecting first failures, counting TypeScript errors, and collecting new specs.
...
Produces a compact digest of factual context for use in loop prompts.
...
Detects whether source code is amenable to property-based testing and provides guidance for it.
...
Drafts and iteratively repairs a local test against a target source using a stack adapter and brain.
...
Retrieves few-shot examples from the library for a given retrieval query.
...
Tokenizes strings, computes set similarity, and selects the most similar trace for a task and stack.
...
Injects mock setup runes into the loop based on a mock plan.
...
Synthesizes mock bundles for modules by sampling schemas and leveraging OpenAPI specs.
...
Materializes fixtures, captures and persists mock contracts, and reads existing contracts from disk.
...
Writes mock handlers and fixtures to disk as a mock store.
...
Orchestrates mock plan and chain building while re-exporting core mock graph types and helpers.
...
Generates tests end-to-end through the loop and returns the run outcome.
...
Provides the CLI command entry point for triggering test generation.
...
Renders module graphs as Mermaid diagrams, ASCII art, or textual summaries.
...
Provides the CLI command entry point for rendering module dependency graphs.

- depends on: src/mock/index.ts, src/mock/render.ts

### src/cli/history.ts — util

Provides the CLI command entry point for inspecting loop or generation history.
...
Provides the CLI command entry point for analyzing the impact of changes on specs.

- depends on: src/adapters/registry.ts, src/git.ts, src/mock/graph.ts, src/loop/impact.ts

### src/distill/improve.ts — util

Provides utilities for model distillation cycles, including cycle scheduling checks, evaluation comparison, and model pointer persistence.
...
CLI entry point that orchestrates the distillation improvement cycle workflow.

- depends on: src/distill/improve.ts

### src/visual/capture.ts — util

Captures screenshots of a URL and saves them to a specified output path.
...
Sends a captured PNG to a vision model with system and user prompts to get a textual response.
...
Extracts code fences from text and runs an iterative visual improvement loop using screenshots and vision feedback.
...
CLI entry point for triggering the visual or distillation improvement process.

- depends on: src/visual/improve.ts

### src/cli/init.ts — util

CLI entry point for initializing project configuration and scaffolding.

- depends on: src/adapters/registry.ts

### src/cli/learn.ts — util

CLI entry point for running the learning/training workflow.

- depends on: src/adapters/registry.ts, src/audit/core.ts, src/library/store.ts

### src/cli/mfe-audit.ts — util

CLI entry point for auditing micro-frontend module compliance and violations.

- depends on: src/mfe/scan.ts, src/mfe/standards.ts, src/factory/report.ts

### src/mfe/contract.ts — util

Generates contract test files for micro-frontend remote and host module exposure points.
...
Scans a directory to plan and discover micro-frontend contract definitions.
...
CLI entry point for generating and managing micro-frontend contracts.

- depends on: src/mfe/contract-scan.ts

### src/mfe/report.ts — util

Aggregates micro-frontend driver results, version alignment violations, and timestamps into a unified report.
...
Runs the micro-frontend driver to execute contract tests and collect compliance results.
...
CLI entry point for running micro-frontend validation and reporting.

- depends on: src/mfe/driver.ts, src/factory/report.ts, src/mfe/standards.ts

### src/cli/migrate.ts — util

CLI entry point for migrating project configurations or code between versions.

- depends on: src/cli/path-cli.ts

### src/cli/mock.ts — util

CLI entry point for generating or running mock services and fixtures.

- depends on: src/mock/index.ts

### src/cli/otel.ts — fetcher 🌐

CLI entry point for fetching and displaying OpenTelemetry trace and metric data.

- depends on: src/cost/ledger.ts, src/util/state.ts, src/observe/otel.ts

### src/cli/peek.ts — util

CLI entry point for inspecting or previewing project state and artifacts.

- depends on: src/loop/events.ts, src/loop/observe.ts

### src/loop/rune-descriptions.ts — util

Provides metadata and human-readable descriptions for pipeline runes and hooks.
...
Describes and renders the pipeline structure including profiles, runes, phases, toggles, and Mermaid diagrams.
...
CLI entry point for displaying and inspecting the configured pipeline structure.

- depends on: src/loop/describe.ts

### src/plan/build.ts — util

Builds a project plan from input targets and specs, identifying untested targets and formatting the plan output.
...
CLI entry point for generating and displaying the project build plan.
...
Converts a quality report into the standard SARIF format object.
...
CLI command handler for running quality checks.
...
CLI command handler for executing code refactoring tasks.

- depends on: src/cli/path-cli.ts

### src/cli/repair.ts — util

CLI command handler for repairing code issues.

- depends on: src/cli/path-cli.ts, src/git.ts

### src/cli/revert.ts — util

CLI command handler for reverting project changes.

- depends on: src/util/git.ts

### src/cli/review.ts — util

CLI command handler for performing code reviews.

- depends on: src/adapters/registry.ts, src/audit/core.ts

### src/cli/run.ts — util

CLI command handler for running the main application loop or task.

- depends on: src/adapters/registry.ts

### src/cli/scan.ts — util

CLI command handler for scanning the codebase or project.

- depends on: src/util/state.ts, src/util/jsonl.ts, src/observe/queue.ts, src/factory/report.ts

### src/cli/serve.ts — util

CLI command handler for starting a local server.

- depends on: src/loop/observe.ts

### src/cost/project.ts — util

Calculates and formats cost projections for model runs.
...
CLI command handler for simulating project costs.
...
Catalogs available skill commands and identifies undocumented ones.
...
Builds skill definitions and wiki command documentation.
...
CLI command handler for managing and building skills.

- depends on: src/skill/build.ts, src/skill/catalog.ts

### src/spec/build.ts — util

Builds project specifications based on provided options.
...
CLI command handler for generating project specifications.

- depends on: src/adapters/registry.ts, src/brain/select.ts, src/spec/build.ts

### src/cli/status.ts — util

CLI command handler for displaying project status.

- depends on: src/adapters/registry.ts

### src/cli/watch.ts — util

CLI command handler for watching files and triggering actions on changes.

- depends on: src/adapters/registry.ts, src/config.ts, src/git.ts

### src/e2e/checkpoint.ts — util

Captures and saves a checkpoint during end-to-end testing.
...
Runs a delegated task within the loop and returns the outcome.
...
Provides a rune that gates loop progression on accessibility checks.
...
Provides a rune that gates loop progression on acceptance criteria.
...
Provides a rune that gates loop progression on a minimum assertion score.
...
Provides a rune that gates loop progression on audit results.
...
Provides a rune that locks loop progression based on behavior verification.
...
Provides a rune that injects execution context metadata distinguishing between unit and end-to-end test kinds.
...
Provides a rune that distills execution traces into a compact, summarized representation.
...
Provides a flake-detection rune that evaluates repeated run signatures against a tolerance threshold to gate flaky tests.
...
Provides a rune that enforces hermetic (isolated, side-effect-free) test execution as a quality gate.
...
Provides a rune that computes test kind, category, score, and slug metadata to promote validated artifacts into the shared library.
...
Provides a rune that gates micro-frontend compatibility checks against a specified design system.
...
Provides a rune that optionally enforces mutation testing thresholds for minimum score and maximum mutant count.
...
Provides a rune that asserts no regressions are introduced relative to a baseline.
...
Provides a rune that validates file path conventions and guards against disallowed or misplaced test paths.
...
Provides a configurable quality-gate rune that applies overridden quality thresholds to the loop.
...
Provides a rune enforcing the red-first TDD discipline by requiring a failing test before implementation.
...
Provides a rune that gates visual regression checks, comparing rendered output against approved baselines.
...
Defines shared TypeScript type definitions used across the loop module's runes and orchestration logic.
