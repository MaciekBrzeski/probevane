...
**Domain map** (logical grouping over the flat `src/` — no folders, see [Architecture](docs/wiki/Architecture.md)): **the loop** `loop`·`brain`·`adapters`·`mock`·`plan` · **gates & analysis** `audit`·`quality`·`arch`·`review`·`a11y`·`mfe`·`visual`·`coverage`·`e2e` · **presentation** `ui`·`tui` (draw through `engine/` facet) · **ops & fleet** `server`·`observe`·`factory`·`doctor`·`ship`·`integrations` · **knowledge & docs** `library`·`distill`·`spec`·`docs`·`skill`·`search` · **composition root** `cli` (imports ~all; stays at root for `bin`/dist dispatch) · **shared leaves** `util`·`cost`·`config.ts`·`git.ts`. Physical regrouping was declined — 735 aliasless relative imports + dist-mirror dispatch make it high-churn for tidiness only.
