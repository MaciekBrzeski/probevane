...
**Domain map** (logical grouping over the mostly-flat `src/`, see [Architecture](docs/wiki/Architecture.md)): **the loop** `loop`·`brain`·`adapters`·`mock`·`planner` · **gates & analysis** `audit`·`quality`·`review`·`mfe`·`visual`·`coverage`·`e2e`·`arch` (structural critique lib — pyramid/metrics/crowding/critique, imported downward by `cli`/`server`/`loop`/`doctor`) · **presentation** `ui`·`tui` (draw through `engine/` facet) · **ops & fleet** `server`·`observe`·`factory`·`spec-run` · **knowledge & docs** `library`·`distill` · **command backends** `commands/` (arch, ship, spec, skill, doctor, search, plan, ado) · **composition root** `cli` (imports ~all; stays at root for `bin`/dist dispatch) · **shared leaves** `util` (incl. `config.ts`, `git.ts`)·`cost`. Full physical regrouping was declined (735 aliasless relative imports + dist-mirror dispatch); only the cheap wins landed — single-command backends into `commands/`, `config.ts`/`git.ts` into `util/`.
...
- **Vane** (`vane/*.vane`, see [Vane](docs/wiki/Vane.md)) is the declarative layer: `commands.vane` = the command surface (catalog.ts is a loader shim; 22 commands run through the `src/vane/run-command.ts` interpreter, shells are 2-line trampolines, handlers in `src/commands/`), `profiles.vane` → generated `src/loop/profiles.gen.ts`, `adapters/*.vane` = adapter data manifests. `probevane vane --check` is the drift gate (`--write` regenerates); `@generated` sources are skipped by quality/graph/arch. NOT a general-purpose language — logic stays TS; Vane grows by declaration kinds, never expressions.
- Generated files are drift-gated, don't hand-edit: `vane/commands.vane` is the command source of truth (via `src/commands/skill/catalog.ts` as loader) → `.claude/skills/probevane/SKILL.md` + `docs/wiki/Commands.md` (CI `skill --check`). `docs/probevane-guide.md` is docs-loop output, reference-integrity gated.
